...
// styled-components
import { Container, EmptyLabel, HeaderContainer, ListContainer, Logo, Wrapper } from './styles'
...
export const Dashboard = () => {
    ...
    const loadPets = async () => {
        ...
        const sortedPets = res.data.sort((a: any, b: any) => b.status - a.status);
    ...
    return (
        <Container>
            ...
            <ListContainer>
                    {list.length > 0 ? (
                        <>
                        {list.map((animal: any) => (<Card callback={updateList} key={animal.id} data={animal}/>))}
                        </>
